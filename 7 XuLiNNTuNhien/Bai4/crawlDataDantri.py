...
base_url = "https://dantri.com.vn"  # Thay bằng URL thực tế
category_url = base_url + "/giao-duc.htm"  # Thay bằng URL chuyên mục thực tế

# Tạo thư mục 'giaoducvnexpress' nếu nó chưa tồn tại
if not os.path.exists("/home/quang/CodePythonMaster/7 XuLiNNTuNhien/Bai4/giaoducdantri"):
    os.makedirs("/home/quang/CodePythonMaster/7 XuLiNNTuNhien/Bai4/giaoducdantri")
...
if response.status_code == 200:
    ...
    article_links = soup.find_all("h3", class_="article-title")  # Thay class bằng class thực tế
    ...
    for i, article in enumerate(articles_info, start=1):
        article_url = category_url + article["URL"]
        article_title = article["Tiêu đề"]

        article_response = requests.get(article_url)
        if article_response.status_code == 200:
            article_soup = BeautifulSoup(article_response.text, "html.parser")
            article_content = article_soup.find("article", class_=["singular-container", "e-magazine"]).get_text()
            filename = f"/home/quang/CodePythonMaster/7 XuLiNNTuNhien/Bai4/giaoducdantri/article_{i}.txt"
            file_path = os.path.join("/home/quang/CodePythonMaster/7 XuLiNNTuNhien/Bai4/giaoducdantri", f"article_{i}.txt")
